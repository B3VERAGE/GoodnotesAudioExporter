...
# NOTA (setup Mattia, 2026-08-12) — gli hook NON stanno piu' qui.
# Sono in ~/.claude/settings.json. Motivo: gli hook dichiarati nel frontmatter di
# una skill sono "scoped to the component's lifetime" (doc Claude Code), cioe'
# girano SOLO mentre la skill e' attiva nella sessione. Dopo un /clear la skill si
# scarica, gli hook smettono e il piano NON rientra da solo — che e' esattamente
# la funzione per cui questa skill esiste. Da settings.json valgono sempre.
# Ogni comando ha davanti la guardia "esiste un piano?", quindi nei progetti
# senza piano non produce nulla. Se aggiorni la skill da upstream, NON
# reintrodurre la chiave hooks: qui.
metadata:
  version: "3.10.0"
---
...
SKILL_DIR="${CLAUDE_PLUGIN_ROOT:-$HOME/.memory/skills/planning-with-files}"
...
## Quick Start (setup Mattia — leggi questo, non il legacy)

**Quando attivarsi:** solo per lavori multi-fase (indicativamente 5+ tool call, o
qualunque cosa che non finisca in un paio di scambi). Per una domanda secca, una
lettura, un fix da una riga **non creare nulla**: tre file di piano per una modifica
banale sono solo rumore nella repo.

**Dove:** mai nella root del progetto. Sempre in `.planning/<data>-<slug>/`, un
sottodirectory per task, così due lavori paralleli non si sovrascrivono:

```bash
~/.memory/skills/planning-with-files/scripts/init-session.sh --plan-dir "Nome del task"
```

Lo script crea i tre file dai template, scrive `.planning/.active_plan` e li rende
risolvibili dagli hook. **Non** passare `--gated` se non lo chiede l'utente: attiva
un gate che può bloccare la chiusura della sessione finché tutte le fasi non sono
complete.

Subito dopo, se il progetto è un repo git e `.gitignore` non menziona già
`.planning`, aggiungi la riga `.planning/` — sono file di lavoro, non storia del
progetto. Se l'utente preferisce versionarli, rispetta la sua scelta.

**`progress.md` è append-only: le voci nuove vanno IN FONDO.** L'hook inietta solo la
coda del file (`tail`), quindi è il fondo che l'agente rivede a ogni messaggio. Non
chiudere il file con tabelle vuote o testo di chiusura: si mangerebbero la coda e
l'iniezione mostrerebbe boilerplate invece del lavoro recente.

Poi, durante il lavoro:

1. **Rileggi il piano prima delle decisioni** — rimette gli obiettivi in attenzione
2. **Aggiorna dopo ogni fase** — stato della fase, errori, file toccati

> **Nota:** i file di piano stanno nel progetto, mai nella cartella della skill.
...
## Rapporto con MEMORY.md (setup Mattia)

Su questa macchina convivono memorie con orizzonti diversi. Non confonderle:

| | `MEMORY.md` (radice progetto) | `.planning/<task>/` |
|---|---|---|
| Orizzonte | tutto il progetto, mesi | il task in corso |
| Contenuto | decisioni, vincoli, **perché** | fasi, checkbox, diario |
| Vita | permanente, potato a mano | usa e getta, si archivia |
| Gestita da | skill `handoff` / `resume` | questa skill |

Regole di travaso:

- Un fatto che **sopravvive al task** (una scelta architetturale, un vincolo, una
  strada provata che non funziona) va in `MEMORY.md`, non solo in `findings.md`.
- A task finito: segna il piano completo, riporta l'essenziale in `MEMORY.md` con
  `/handoff`, poi archivia il piano **spostandolo di un livello**:
  `mkdir -p .planning/archive && mv .planning/<slug> .planning/archive/ && : > .planning/.active_plan`.
  Rinominarlo dentro `.planning/` non lo disattiva: con `.active_plan` vuoto il
  resolver ripiega sulla cartella più recente che contenga un `task_plan.md` e
  ripesca proprio quello. Sotto `archive/` la scansione non lo vede.
- Non duplicare in `.planning/` quello che è già in `MEMORY.md`, e viceversa.
- **Mai credenziali** in nessuno dei due: cita dove stanno, mai il valore.

## Rapporto con MASTER_PLAN.md (setup Mattia — Anti-Desincronizzazione)

Nei progetti che possiedono un `MASTER_PLAN.md` alla radice (generato da `master-project-planner`), i ruoli sono gerarchici e ben distinti:

| File | Ruolo | Durata | Contenuto |
|---|---|---|---|
| `MASTER_PLAN.md` (root) | **Macro-Roadmap & SSOT di Progetto** | Settimane / Mesi | Architettura, 4 Milestone, DoD, Task 1.1...4.X, Audit Gates |
| `.planning/<slug>/task_plan.md` | **Micro-Zoom del Task Corrente** | Singola sessione / task | Step esecutivi (1, 2, 3), tool call, comandi, RAM su disco |

### Regole Tassative di Integrazione e Sincronizzazione:
1. **Divieto Assoluto di Duplicazione**:
   - Se `MASTER_PLAN.md` esiste nella radice, **è severamente vietato ricopiare l'intero piano o tutte le milestone dentro `task_plan.md`**.
   - `task_plan.md` deve contenere SOLO i micro-passi operativi necessari a completare il **singolo task attivo** di `MASTER_PLAN.md`.
2. **Header con Link al Genitore**:
   - In cima a `task_plan.md`, inserisci sempre il riferimento esplicito:
     ```markdown
     # Task Plan: [Nome del Task]
     **Parent**: [MASTER_PLAN.md](../../MASTER_PLAN.md#task-x-y)
     **Milestone**: Milestone X
     **Definition of Done (DoD)**: [Copia il criterio di verifica dal Master Plan]
     ```
3. **Two-Way Sync on Completion (Sincronizzazione Obbligatoria alla Chiusura)**:
   - Quando tutti gli step di `task_plan.md` sono completati e la Definition of Done (DoD) è soddisfatta:
     1. Spunta `[x]` nel `task_plan.md` locale.
     2. **AGGIORNA IMMEDIATAMENTE `MASTER_PLAN.md`**: esegui un edit sul file radice trasformando `- [ ] Task X.Y: ...` in `- [x] Task X.Y: ...`.
     3. Registra in `progress.md`: `Completato Task X.Y e sincronizzato con successo in MASTER_PLAN.md`.
     4. Solo dopo aver aggiornato il genitore `MASTER_PLAN.md`, archivia la cartella in `.planning/archive/`.
