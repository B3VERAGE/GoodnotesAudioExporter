...
### Step 5: Write the agent file & Register in Runtime

1. **Write `<agent-name>.md`** in the `agents/` folder using the exact Antigravity-native structure below. 
   - Antigravity uses native permission booleans (`enable_write_tools`, `enable_mcp_tools`, `enable_subagent_tools`) instead of arbitrary tool strings.
   - For `model`, use `inherit` (recommended) or specific tiers (`pro`, `flash`).

```markdown
---
name: <agent-name>
description: <One-line summary of what this agent does.>
enable_write_tools: <true_if_writes_files_or_runs_terminal_else_false>
...
2. **Register Immediately in Runtime (`define_subagent`)**:
   After writing the file to disk, **immediately invoke the tool `define_subagent`** so the new subagent is instantly available for `invoke_subagent` in the current session without requiring a restart:
   - `name`: `<agent-name>`
   - `description`: `<One-line summary>`
   - `system_prompt`: `<The complete persona and instructions from the .md file>`
   - `enable_write_tools`: `true` (if the agent needs to write/edit files or run terminal commands) or `false`
   - `enable_mcp_tools`: `true`
   - `enable_subagent_tools`: `false`
...
### Step 6.5: Sync Subagent into AGENTS.md (MANDATORY)

Every created subagent MUST be registered in the project's `AGENTS.md` so that future sessions and other agents know of its existence, capabilities, and delegation rules.

1. **Locate `AGENTS.md`**:
   - Check if `AGENTS.md` exists in the project root (`git rev-parse --show-toplevel` or current workspace root).

2. **If `AGENTS.md` ALREADY EXISTS**:
   - Inspect `AGENTS.md` for a `## Subagents & Roles` section.
   - If the section exists: append or update the table row for `<agent-name>`.
   - If the section does not exist: insert the section cleanly right after `## Active Skills & Playbooks` (or under `## Project Overview`):
     ```markdown
     ## Subagents & Roles

     The following specialized subagents are configured for this project:

     | Subagent | Role & Purpose | Model | Tools | When to Delegate |
     | :--- | :--- | :--- | :--- | :--- |
     | `<agent-name>` | <One-line summary of role> | `<model>` | `<tools>` | <Exact condition/trigger to delegate> |
     ```
   - **Do not overwrite** any existing project conventions or rules.

3. **If `AGENTS.md` DOES NOT EXIST**:
   - Create `AGENTS.md` from scratch:
     - Detect the project stack (package manager, language, primary frameworks).
     - Write the essential project overview, setup/verification commands, and conventions.
     - Include the `## Subagents & Roles` section containing the newly created subagent.
     - Inform the user that `AGENTS.md` was scaffolded and recommend running `agent-rules-generator` to generate comprehensive rule files in `.agents/rules/`.
